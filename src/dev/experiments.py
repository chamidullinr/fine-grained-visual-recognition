...
def create_config(*, model, loss, data, **kwargs):
    ...
    # check if model name exists and then add suffix
    # if path is not None:
    #     i = 1
    #     while (os.path.isfile(os.path.join(path, specs_name)) or
    #             os.path.isfile(os.path.join(path, history_name))):
    #         history_name = f'{model_name}_{i}.csv'
    #         specs_name = f'{model_name}_{i}.json'
    #         i += 1
    if 'path' in kwargs:
        del kwargs['path']
