...
__all__ = ['clean_country_map', 'country_f1_score', 'CountryF1Score']
...
def country_f1_score(preds, targs, country_weights):
    ...
    if len(preds.shape) == 2:
        preds = preds.argmax(1)
    assert np.all(preds < no_labels), f'Unexpected class (label) in prediction array: {preds[preds >= no_labels]}'
    assert np.all(targs < no_labels), f'Unexpected class (label) in target array: {targs[targs >= no_labels]}'
    ...
    f1_country = (f1.reshape(-1, 1) * country_weights).sum(axis=0) / (country_weights.sum(axis=0) + 1e-10)
...
class CountryF1Score:
    def __init__(self, country_weights):
        self.country_weights = country_weights
        self.__name__ = 'country_f1_score'
    
    def __call__(self, preds, targs):
        return country_f1_score(preds, targs, self.country_weights)
